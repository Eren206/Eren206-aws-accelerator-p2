...
### Frontend
This repo now includes a lightweight TypeScript frontend in `frontend/`.
The build emits static assets to `dist/public/`, and the Hono server serves the
UI from `/` while keeping the API routes at `/documents`, `/annotations`, and
`/health`. The `public/` folder is ignored because it is only a static output
folder if a different deploy flow creates it.
...
In the UI, leave "API base" blank for same-origin requests. When using a
deployed API Gateway URL, set the API base and optional API key in the
connection panel.

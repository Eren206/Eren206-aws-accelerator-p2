...
export class DocumentsService {
  ...
  async getDocuments() {
    ...
    try {
      ...
      return shouldUseLocalFallback() ? getLocalDocuments() : [];
    }
  }
  ...
  async getDocument(id: string) {
    ...
    try {
      ...
      console.error('Error fetching document from AWS resources', error);
  ...
  async triggerAnalysis(docId: string, s3Key: string) {
    ...
    if (!queueUrl || !bucketName) {
      console.warn(
        'Queue URL or Bucket Name not configured. Using local analysis fallback.',
      );
      runLocalAnalysis(docId);
      return;
    }
    ...
    try {
      ...
      if (shouldUseLocalFallback()) {
        console.warn('SQS request failed. Using local analysis fallback.', error);
        runLocalAnalysis(docId);
        return;
      }
      throw error;
